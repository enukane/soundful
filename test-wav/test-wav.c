...
void
create_and_write_data(FILE *fp, int n, double start_freq, double end_freq, double amp)
{
    int i;
    size_t ret;

    for (i = 0; i < n; i++) {
        //int16_t s = (int16_t) round(chirp(i, n, (double)SAMPLE48KHZ, start_freq, end_freq, amp)); 
        //int16_t s = (int16_t) (amp * sin(2.0 * M_PI * start_freq * i / (double)SAMPLE48KHZ)); 
        int16_t s = (int16_t) sinwave(i, n, (double)SAMPLE48KHZ, start_freq, amp);
        //printf("%d: %d\n", i, s);
...
usage()
{
    printf("usage: test-wav [start] [end] [amp] [n]");
}

int
main(int argc, char *argv[])
{
    int start_freq, end_freq, amp, n;
    size_t ret;
    FILE *fp;
    ...
    start_freq = atoi(argv[1]);
    end_freq = atoi(argv[2]);
    amp = atoi(argv[3]);
    n = atoi(argv[4]);

    printf("start=%d end=%d amp=%d n=%d\n", start_freq, end_freq, amp, n);
    ...
    create_and_write_data(fp, n, (double)start_freq, (double)end_freq, (double)amp);
